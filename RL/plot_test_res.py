...
def action_plot(action_name, action_data, action_opt):
    ...
    #其他: 16个
    figure = plt.figure(figsize=(44, 8))
    plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
    index = 1
...
def state_plot(state_data, next_state_opt):
    ...
    sub_plot_fuc(row=3, col=1, plot_num=1, name='Efficiency', true=state_data['1号机组锅炉效率'], opt=opt_effi,
    ...
    sub_plot_fuc(row=3, col=1, plot_num=2, name='Nox', true=state_data['SCR反应器入口Nox含量'], opt=opt_nox,
                 color_true='thistle', color_opt='red')
    plt.xlabel('time (min)', fontsize=20)
    plt.ylabel('mg/m^3', fontsize=20)
    ...
    sub_plot_fuc(row=3, col=1, plot_num=3, name='Load', true=state_data['#1机组锅炉负荷'], opt=opt_load,
                 color_true='thistle', color_opt='red')
    plt.xlabel('time (min)', fontsize=20)
...
def plot_all():
    ...
    action_data = pd.read_csv('result/test_result_action_data.csv')
    action_opt = pd.read_csv('result/test_result_action_opt.csv')
    state_data = pd.read_csv('result/test_result_state_data.csv')
    next_state_opt = pd.read_csv('result/test_result_next_state_opt.csv')
